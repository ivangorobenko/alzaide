...
export const configureMessageRoutes = (commandBus: CommandBus, queryBus: QueryBus) => {
    messageRouter.put('/message',
        (req: Request, res: Response) => new MessageController(commandBus, queryBus).laisserMessage(req, res)
    )
    messageRouter.get('/messages',
        (req: Request, res: Response) => new MessageController(commandBus, queryBus).recupererMessages(req, res)
    )
